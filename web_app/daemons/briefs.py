...
def extract_briefs(document, a):
    ...
    for i in list_briefs:
        try:
            ...
            Debug.get_exception(sub_system='engine_feed', severity='error', tags='get_link_brief', data=a['base_link'])
            link = None
        try:
            if a['brief_ro_title']:
            ...
            Debug.get_exception(sub_system='engine_feed', severity='error', tags='get_ro_title_brief', data=a['base_link'])
            ro_title = None
        try:
            title = i.select_one(a['brief_title']).text.encode('utf-8').strip()
        except:
            Debug.get_exception(sub_system='engine_feed', severity='error', tags='get_title_brief', data=a['base_link'])
            title = None
        try:
            summary = i.select_one(a['brief_summary']).text.encode('utf-8').strip()
        except:
            Debug.get_exception(sub_system='engine_feed', severity='error', tags='get_summary_brief', data=a['base_link'])
            summary = None
        try:
            ...
            Debug.get_exception(sub_system='engine_feed', severity='error', tags='get_thumbnail_brief', data=a['base_link'])
            thumbnail = None
