...
function set_news(__news, __view, __font, __size){
    ...
    if(__view == "list_view"){
        ...
        for(j = 0; j < __news.length; j++){
            make_news_column_list_view(__news[j], __font, __size);
            if(j == 0)
                make_news_column_detail_view(__news[j]);
        }
    }else if(__view == "details_view"){
        for(j = 0; j < __news.length; j++){
            make_news_detail_view(__news[j], __font, __size);
        }
    }else if(__view == "details_pic_view"){
        for(j = 0; j < __news.length; j++){
            make_news_detail_pic_view(__news[j], __font, __size);
        }
