...
function make_news_list_view(news) {
    ...
    if (news['options']['note'] != false) {
        _make.find('div#note_icon').addClass('colorBlue').attr('data-news', news['id']).attr('data-action', news['id']);
    }else{
        _make.find('div#note_icon').attr('data-news', news['id']).attr('data-action', news['id']);
    }
