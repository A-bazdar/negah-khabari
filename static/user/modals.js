            ...
            success: function (response) {
                ...
                if (status) {
                    ...
                    if(modal == "UserManagement"){
                        $("select.change-status.user-management").select2().on("change", function () {
                            var status = $(this).select2("val");
                            var user_id = $(this).attr("data-user");
                            var postData = [
                                {name: 'status', value: status},
                                {name: 'user_id', value: user_id},
                                {name: 'action', value: 'change_status'},
                                {name: '_xsrf', value: xsrf_token}
                            ];
                            jQuery.ajax(
                            {
                    ...
                    if(modal == "PatternSearch"){
                        ...
                        $("form#pattern_search_form .pattern-search[name=period]").select2().on("change", function () {
